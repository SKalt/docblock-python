  ...
  return {
    ...
    lint(textEditor) {
      ...
      return new Promise(function(resolve) {
        resolve(
          that.lint_docblocks().map(missing => {
            return that.formatLint(
              missing.pos,
              missing.par,
              missing.type,
              editorPath
...
export function extract_parameters(query) {
  ...
  let args_list = scanArgs(args)
    .filter(x => {
      return x.trim() !== "self" && x.trim() !== "*" && x.trim().length > 0;
    })
    .map(x => {
...
function generate_docblock() {
  ...
  let docblock;
  // Load settings:
  let [tab_length, tab_type] = ["editor.tabLength", "editor.softTabs"].map(q =>
    atom.config.get(q, { scope: ["source.python"] })
  );
        ...
        if (logic) {
          ...
          if (!header) {
            atom.notifications.addWarning(
              "Something went wrong." + "Reached end of file."
            );
            return null;
          }
          query = header[0];
          ...
          if (query.match(/def|async def/)) {
            ...
            options.default_desc_text = atom.config.get(
              "docblock-python.default_desc_text"
            );
...
function formatReturn(return_type, returns, options) {
  if (options.use_types) {
    if (options.style == "sphinx" || options.style == "epytext") {
      returns = returns.map(v => v.replace(" type", " " + return_type));
    } else {
      returns = returns.map(v => v.replace("type", return_type));
    }
  } else {
...
function process_def(query, options) {
  ...
  if (options.returns) {
    let return_txt = options.default_desc_text
      ? returns.join(tabs)
...
function process_list(args, options, that, label = "parameter") {
  ...
  let lines = that.format_lines(args_, options, label).join("\n") + "\n";
  return lines;
}
...
function get_decorators(start, lines = -1, accum = []) {
  ...
  if ((editor = atom.workspace.getActiveTextEditor())) {
    ...
    let logic = this.stop_words.map(x => line.search(x)).some(x => x >= 0);
...
function process_dataclass(start, lines = 1, accum = [[], []]) {
  ...
  if ((editor = atom.workspace.getActiveTextEditor())) {
    let line = editor.lineTextForBufferRow(start.row + lines);
    let logic = ["def"].map(x => line.search(x)).some(x => x >= 0);
    if (logic) {
      return accum;
    } else if (line.match(":")) {
  ...
  if (attributes.length) {
    ...
    if (short_attrs.length) {
      ...
      docblock += short_attrs
        .map(x =>
          ["numpy", "sphinx", "epytext"].indexOf(options.style) > -1
    ...
    while (match) {
      ...
      if (Object.keys(def).length && last_end < def.pos.row) {
        let block = {
          ...
          pos_end: editor.buffer.positionForCharacterIndex(
            dbRegex.lastIndex + 1
          ),
          def: def.def[0],
          def_lines: def.def[1],
